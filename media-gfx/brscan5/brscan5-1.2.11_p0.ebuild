...
src_install() {
	...
	for l in libLxBsNetDevAccs.so.1.0.0 libLxBsScanCoreApi.so.3.2.0 libLxBsUsbDevAccs.so.1.0.0 libLxBsDeviceAccs.so.1.0.0 libsane-brother5.so.1.0.7
	do
		v=$(echo "$l" | sed 's/^.*\.so\.//')
		p=$(basename "$l" ".$v")
	...
	# ???
	dosym "libLxBsScanCoreApi.so.3.2.0" "usr/lib64/libScanCoreApi.so"
